"""v0.5: capacity-pressure ablation.

v0 ablation found that at 14 nodes / 24 edges, the no_growth variant
beat full SOMA on all 4 regimes — base capacity was sufficient for
the task. This script stresses capacity: 8 regimes of varying
frozen-MLP complexity (depth 2-4, hidden 16-64), 500 steps each.
Hypothesis: growth pays off when learner capacity is actually
saturated.

Runs {full, no_growth, no_consol, no_hebbian} + OnlineMLP baseline
on the same capacity schedule. Reports per-regime MSE and
final graph size per variant.
"""
...
def run_online_mlp(
    ...
    return VariantResult("online_mlp", mses, regimes, [0.0], [0.0], dt)
...
def main() -> None:
    ...
    variant_order = ["full", "no_growth", "no_consol", "no_hebbian", "online_mlp"]
    for v in variant_order:
        print(f"{v:<14}", end="")
    print()
    for regime_idx, regime in enumerate(schedule.regimes):
        print(f"  {regime.name:<14}", end="")
        for v in variant_order:
    ...
    print("\n" + "=" * 78)
    print("FINAL GRAPH SIZE BY SOMA VARIANT")
    print("=" * 78)
    ...
    out_path = "research/developmental/results/env_sequence_v05_capacity.json"
