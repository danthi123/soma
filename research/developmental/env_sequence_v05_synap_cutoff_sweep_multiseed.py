"""v0.5 locality-cutoff sweep — find the sweet spot for synap_local.

The ``synap_local`` finding (commit 2ba566b) established that a hard
positional-locality filter at ``max_distance=0.5`` turns synaptogenesis
from harmful into beneficial. The 0.5 cutoff was picked by back-of-
envelope analysis of the position-init scale (``randn * 0.1``; typical
pair distances 0.3-1.0). This sweep characterizes the effect across a
range of cutoffs — does the benefit peak at 0.5, taper, invert?

Variants (6):
- ``no_growth``         baseline.
- ``synap_only``        unfiltered synap (0.0 = disabled filter).
- ``synap_cutoff_025``  tight filter, ~quartile of typical distances.
- ``synap_cutoff_050``  reference value (known positive, matches prior).
- ``synap_cutoff_075``  wider filter, admits most pairs.
- ``synap_cutoff_100``  loose filter, filter barely active.

Expected outcome: an inverted-U. Too tight (0.25) and few edges can
form at all — low coverage. Too loose (1.0) and the filter admits
nearly everything, reproducing ``synap_only``'s harm. 0.5 is the
reference operating point.

Seeds {0, 1, 42}. 6 variants x 3 seeds = 18 runs, ~80-100 min.

Note on position_dim: SOMAConfig default uses position_dim=3 with
jitter ``randn * 0.1``. The expected norm of ``randn(3) * 0.1`` is
``sqrt(3) * 0.1 ~= 0.173``. Pair distance between two such draws has
expected norm ``~0.24``. Cutoff values below ~0.15 would reject
almost all pairs; cutoffs above ~0.50 admit most pairs. Values
chosen span the interesting range.
"""
...
# Variant name -> (cutoff_value)  where 0.0 = no filter
CUTOFFS: dict[str, float] = {
    "synap_cutoff_025": 0.25,
    "synap_cutoff_050": 0.50,
    "synap_cutoff_075": 0.75,
    "synap_cutoff_100": 1.00,
}
...
def build_config(dim: int, variant: str, seed: int) -> SOMAConfig:
    ...
    elif variant in CUTOFFS:
        overrides.update(
            neurogenesis_interval=0,
            synaptogenesis_max_distance=CUTOFFS[variant],
        )
    else:
...
def main() -> None:
    ...
    variants = [
        "no_growth",
        "synap_only",
        "synap_cutoff_025", "synap_cutoff_050",
        "synap_cutoff_075", "synap_cutoff_100",
    ]
    ...
    # Win-count vs synap_only
    for treat in ["synap_cutoff_025", "synap_cutoff_050",
                  "synap_cutoff_075", "synap_cutoff_100"]:
        base = "synap_only"
        print()
        print("=" * 90)
    ...
    out_path = (
        ...
        "env_sequence_v05_synap_cutoff_sweep_multiseed.json"
    )
