...
``scripts/demo_memory_inspect.py``. It was moved here so ``pip install
soma-memory`` users who run ``soma stats`` / ``soma search`` don't hit
``ModuleNotFoundError: No module named 'scripts'`` — ``scripts/`` is
not part of the wheel. The repo-root file still exists as a thin
wrapper for ``python scripts/demo_memory_inspect.py …`` use from a
clone.
